...
def objective(trial):
    ...
    params = {
        ...
        # "max_features": trial.suggest_categorical("max_features", ["sqrt", "log2", 2, 3, 4, 5, 6]),
        "bootstrap": trial.suggest_categorical("bootstrap", [True, False]),
        "criterion": trial.suggest_categorical("criterion", ["squared_error", "friedman_mse"]),
    }

    # Instantiate the CatBoostRegressor with the suggested hyperparameters
    model = ExtraTreesRegressor(**params, random_state=42, n_jobs=4)
...
def print_progress_callback(study, trial):
    ...
    with open("tuning_results_ExtraTrees.txt", "a", encoding="utf-8") as f:
